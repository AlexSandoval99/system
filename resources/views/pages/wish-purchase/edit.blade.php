...
@section('title', 'Editar Pedido de Compras')
@section('content')
<div class="row">
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            {{ Form::open(['route' => ['wish-purchase.update', $wish_purchase->id], 'method' => 'PUT']) }}
            <div class="ibox-content">
                ...
                <div class="row">
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input class="form-control" type="text" name="requested_by" value="{{ old('requested_by', $wish_purchase->user->name) }}" disabled>
                    </div>
                    <div class="form-group col-md-2">
                        <label>Sucursal</label>
                        {{ Form::select('branch_id', $branches, old('branch_id',$wish_purchase->branch_id), ['class' => 'form-control', 'select2', 'id' => 'branch_id']) }}
                    </div>
                    <div class="form-group col-md-2">
                        <label>Fecha</label>
                        <input class="form-control" type="text" name="date" value="{{ old('date', $wish_purchase->date) }}" readonly>
                    </div>
                </div>
            </div>
            <div class="ibox float-e-margins">
            ...
            <div class="ibox-footer">
                ...
                <a href="{{ url('wish-purchase') }}" class="btn btn-sm btn-danger">Cancelar</a>
            </div>
            {{ Form::close() }}
        </div>
    </div>
...
@section('layout_js')
    <script>
        ...
        {
            @foreach($wish_purchase->wish_purchase_details as $wish)
                addToTable(
                    {{$wish->material_id}}, 
                    '{{$wish->material->description}}', 
                    {{$wish->quantity}}, 
                    {{$wish->presentation}}, 
                    '{{config('constants.presentation.' . $wish->presentation)}}', 
                    '{{$wish->material->description}}', 
                    '{{$wish->material->description}}');
            @endforeach
        ...
        {
            console.log(id, name, quantity, presentation_id, presentation_name, description,product_name);
            counter++;
