...
class VoucherController extends Controller
{
    public function index()
    {
        ...
        $boxes = VoucherBox::with('branch', 'stamped')->whereNotNull('stamped_id')->orderBy('branch_id')->orderBy('voucher_number')->get();

        $voucher_boxes = [];
        foreach ($boxes as $key => $box)
        {
            $voucher_boxes[$box->id] = $box->voucher_fullnumber_branch;
        }
        $vouchers = Voucher::with('branch', 'voucher_box', 'stamped', 'client.emails')
        ...
        if(request()->s)
        {
            $vouchers = $vouchers->where(function($query){
                $query->where('razon_social', 'LIKE', '%' . request()->s . '%')
                ->orWhere('ruc', 'LIKE', '%' . request()->s . '%')
                ->orWhere('voucher_fullnumber', request()->s);
            });
        }
        if(request()->client_id)
        {
            $vouchers = $vouchers->where('client_id', request()->client_id);
        }

        if(request()->voucher_number)
        {
            $vouchers = $vouchers->where('voucher_number', request()->voucher_number);
        }

        if(request()->voucher_box_id)
        {
            $vouchers = $vouchers->where('voucher_box_id', request()->voucher_box_id);
        }

        if(request()->amount)
        {
            $vouchers = $vouchers->where('amount', request()->amount);
        }
    ...
    public function previous_create()
    {
        $clients = [];
        $vouchers = [];
        if(request()->document_number || request()->name)
        {
            $clients = Client::select('clients.*','clients.id as client_id')
                ->where('clients.status', true)
                ->whereNotExists(function($query){
                    $query->select(DB::raw(1))
                        ->from('contract_clients')
                        ->join('contracts', 'contract_clients.contract_id', '=', 'contracts.id')
                        ->whereRaw('contract_clients.client_id = clients.id AND contract_clients.status = 1 AND contracts.status = 5');
                });
            $client_contracts = Client::select('clients.*')
                ->leftjoin('budget_production', 'contract_clients.client_id' , '=', 'clients.id')
                ->leftjoin('contracts', 'contracts.id' , '=', 'contract_clients.contract_id')
                ->leftjoin('enterprises'      , 'contracts.enterprise_id'     , '=', 'enterprises.id')
                ->where('clients.status', true)
                // ->whereIn('contracts.enterprise_id', [4,10])
                ->whereIn('contracts.status', [1,2,5,10]);
            if(request()->document_number)
            {
                $document_number = str_replace('.','',request()->document_number);
                $clients = $clients->where(function($query) use ($document_number){
                                        $query->orWhere('clients.ruc', 'LIKE', '%' . $document_number . '%')
                                            ->orWhere('document_number', 'LIKE', '%' . $document_number . '%');
                                    });

                $client_contracts = $client_contracts->where(function($query) use ($document_number){
                    $query->orWhere('clients.ruc', 'LIKE', '%' . $document_number . '%')
                        ->orWhere('document_number', 'LIKE', '%' . $document_number . '%');
                });
            }
            ...
            if(request()->name)
            {
                $name_filter = str_replace(" ", "%",request()->name);
                $clients     = $clients->where(function($query) use ($name_filter){
                                            $query->whereRaw("CONCAT(first_name, ' ', last_name) LIKE ?", ["%" . $name_filter . "%"]);
                                });

                $client_contracts = $client_contracts->where(function($query) use ($name_filter){
                    $query->orwhereRaw("CONCAT(first_name, ' ', last_name) LIKE ?", ["%" . $name_filter . "%"])
                            ->orWhere('contracts.denomination', 'LIKE', '%' . $name_filter . '%');
                });
            }

            $query = 'IF((SELECT 1 from contracts where contracts.account_holder_id = clients.id AND contracts.enterprise_id = 1 AND contracts.status NOT IN (3,4,6,7) limit 1) AND (SELECT 1 from contracts where contracts.account_holder_id = clients.id AND contracts.enterprise_id = 2 AND contracts.status NOT IN (3,4,6,7) limit 1),1,0) as both_enterprise';
            $clients = $clients->selectRaw($query);

            $client_contracts = $client_contracts->selectRaw($query);

            $client_contracts = $client_contracts->groupBy('contract_id')->get();
            $clients = $clients->get();
            $clients = $clients->merge($client_contracts);
        }
        ...
        if (request()->ajax())
        {
            ...
            if ($purchases->count() > 0)
            {
                foreach ($purchases as $key => $purchase)
                {
                    ...
                    {
                        $results['items'][$key]['products'][$key2]['id']              = $detail_products->material_id;
